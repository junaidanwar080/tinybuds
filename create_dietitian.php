...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        $sql = "INSERT INTO dietitian (Name, Email, Password, Certificate) VALUES ('$name', '$email', '$hashedPassword', '$certificate')";
        if ($conn->query($sql) === TRUE) {

            // After successfully adding a new dietitian
            header('location: Dietitian.php?success=add');
            exit;
        } else {
...
<body>
    <div class="container mt-5">
        <h1 class="mb-4">Create Dietitian</h1>
        <?php if (!empty($errors)): ?>
        ...
        <?php endif; ?>
        <form action="create_dietitian.php" method="POST">
